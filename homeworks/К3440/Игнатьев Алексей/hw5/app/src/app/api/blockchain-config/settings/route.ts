...
export async function GET() {
  const upstream = await upstreamFetch(`/blockchain-config/settings`, { method: "GET" });
...
export async function PUT(req: Request) {
  ...
  const upstream = await upstreamFetch(`/blockchain-config/settings`, { method: "PUT", body: JSON.stringify(body) });
  const json = await upstream.json().catch(() => null);
  return withCookies(upstream, json, upstream.status);
}
