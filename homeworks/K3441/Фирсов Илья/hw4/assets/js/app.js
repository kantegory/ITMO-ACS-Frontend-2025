...
function updateThemeToggleButtons(theme) {
  document.querySelectorAll('[data-theme-toggle]').forEach((btn) => {
    const next = theme === 'dark' ? 'Светлая' : 'Тёмная';
    btn.textContent = next;
    btn.setAttribute('aria-pressed', theme === 'dark' ? 'true' : 'false');
    btn.setAttribute('aria-label', `Переключить тему: сейчас ${theme === 'dark' ? 'тёмная' : 'светлая'}`);
    btn.title = `Включить ${next.toLowerCase()} тему`;
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  updateThemeToggleButtons(document.documentElement.getAttribute('data-theme') || initialTheme);
