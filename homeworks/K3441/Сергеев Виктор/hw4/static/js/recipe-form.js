...
function addIngredientRow(initialData = null) {
    ...
    const amountId = `ingredient-amount-${rowIndex}`;
    const unitId = `ingredient-unit-${rowIndex}`;
    row.innerHTML = `
        <div class="row g-3 align-items-end">
            <div class="col-md-5">
                <label for="${ingredientId}" class="form-label">Ингредиент*</label>
                <select id="${ingredientId}" class="form-select ingredient-select" required 
                        aria-required="true" aria-label="Выберите ингредиент">
            ...
            <div class="col-md-3">
                ...
                <input id="${amountId}" type="number" class="form-control ingredient-quantity" min="0.1" step="0.1" required
                       aria-required="true" aria-label="Количество ингредиента">
            </div>
            <div class="col-md-3">
                <label for="${unitId}" class="form-label">Единица измерения*</label>
                <input id="${unitId}" type="text" class="form-control ingredient-unit" required
                       aria-required="true" aria-label="Единица измерения ингредиента">
            </div>
            <div class="col-md-1 text-end">
                <button type="button" class="btn btn-outline-danger" aria-label="Удалить ингредиент из списка">
                    &times;
                </button>
            </div>
        </div>
...
function addTagRow(initialData = null) {
    ...
    row.setAttribute('role', 'listitem');
    const rowIndex = container.children.length + 1;
    const tagId = `tag-select-${rowIndex}`;
    row.className = "tag-row";
    row.innerHTML = `
        <div class="row g-3 align-items-end">
            <div class="col-md-5">
                <label for="${tagId}" class="form-label">Тег*</label>
                <select id="${tagId}" class="form-select ingredient-select" required 
                        aria-required="true" aria-label="Выберите тег">
                    <option value="">Выберите</option>
            ...
            <div class="col-md-1 text-end">
                <button type="button" class="btn btn-outline-danger" aria-label="Удалить тег из списка">
                    &times;
                </button>
            </div>
        </div>
...
function addStepRow(initialData = null) {
    ...
    row.setAttribute('role', 'listitem');
    const rowIndex = container.children.length + 1;
    const instructionId = `step-instruction-${rowIndex}`;
    row.innerHTML = `
        <div class="d-flex justify-content-between align-items-center mb-2">
            <strong aria-label="Шаг ${rowIndex}">Шаг ${rowIndex}</strong>
            <button type="button" class="btn btn-sm btn-outline-danger" 
                    aria-label="Удалить шаг из списка">&times;</button>
        </div>
        <div class="mb-3">
            <label for="${instructionId}" class="form-label">Описание шага*</label>
            <textarea id="${instructionId}" class="form-control step-instruction" rows="3" required
                      aria-required="true" aria-label="Описание шага ${rowIndex} приготовления"></textarea>
        </div>
    `;
    row.querySelector("button").addEventListener("click", () => row.remove());
    if (initialData) {
