                    ...
                    <div class="recipe-stats" role="group" aria-label="Статистика рецепта">
                        <span class="me-3" aria-label="${likes} лайков">
                            <svg class="icon text-danger" aria-hidden="true" width="16" height="16">
                                <use href="images/icons-sprite.svg#icon-heart-fill"></use>
                            </svg> ${likes}
                        </span>
                        <span aria-label="${comments} комментариев">
                            <svg class="icon text-primary" aria-hidden="true" width="16" height="16">
                                <use href="images/icons-sprite.svg#icon-chat-dots"></use>
                            </svg> ${comments}
                        </span>
                    </div>
                </div>
        ...
        const commentHtml = `
            <div class="comment-item" style="margin-left: ${marginLeft}px">
                ...
                <div class="comment-content">
                    ...
                    <p class="comment-text">${comment.content}</p>
                    ${level < 3 ? '<button class="btn btn-link btn-sm p-0 comment-reply-btn">Ответить</button>' : ''}
                </div>
            </div>
        `;
    ...
    return `
        <div class="post-card mb-4" data-post-id="${post.id}">
            ...
            <div class="post-meta mt-3">
                ${hasImages ? `<span class="badge bg-secondary me-2"><svg class="icon" width="14" height="14"><use href="images/icons-sprite.svg#icon-images"></use></svg> ${post.images.length} фото</span>` : ''}
                ${hasRecipes ? `<span class="badge bg-secondary me-2"><svg class="icon" width="14" height="14"><use href="images/icons-sprite.svg#icon-book"></use></svg> ${post.recipes.length} рецепт(а)</span>` : ''}
            </div>
            <div class="post-footer mt-3 pt-3 border-top">
                <div class="d-flex justify-content-between">
                    <div class="post-stats">
                        <span class="me-3">
                            <svg class="icon" aria-hidden="true" width="16" height="16"><use href="images/icons-sprite.svg#icon-heart"></use></svg> ${Math.floor(Math.random() * 200 + 50)}
                        </span>
                        <span>
                            <svg class="icon" aria-hidden="true" width="16" height="16"><use href="images/icons-sprite.svg#icon-chat-dots"></use></svg> ${Math.floor(Math.random() * 50 + 5)}
                        </span>
                    </div>
