...
- Темизация через CSS-переменные (светлая/тёмная тема) с учётом системной настройки и ручным переключателем в шапке
- SVG-спрайт для иконок (файл `assets/icons.svg`, подключается через `<use href="assets/icons.svg#icon-name">`)
...
- Иконки находятся в `assets/icons.svg` и переиспользуются на всех страницах
- Пример вставки: `<svg class="icon icon-md"><use href="assets/icons.svg#icon-home"></use></svg>`
- Для стилизации есть утилитные классы: `.icon`, `.icon-sm`, `.icon-md`, `.icon-lg`, `.icon-inline`
